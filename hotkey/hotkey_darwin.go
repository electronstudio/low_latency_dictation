...
// darwinModMap maps our platform-agnostic Modifier bits to the macOS
// (Carbon/CGEventFlags) modifier values used by golang.design/x/hotkey.
var darwinModMap = map[Modifier]gohk.Modifier{
	ModCtrl:  gohk.ModCtrl,
	ModShift: gohk.ModShift,
	ModAlt:   gohk.ModOption,
	ModSuper: gohk.ModCmd,
}

// darwinKeyMap maps our platform-agnostic Key to macOS virtual key codes.
var darwinKeyMap = map[Key]gohk.Key{
...
func (h *Hotkey) register() error {
	key, ok := darwinKeyMap[h.key]
	if !ok {
		return unsupportedKeyErr(h.key)
	}
	mods := nativeMods(h.mods, darwinModMap)
